<#
.SYNOPSIS
    ローカルビルド済みのハンズオン環境コンテナイメージ（latest）を起動する。

.DESCRIPTION
    Build-Image.ps1 でビルドした `latest` タグのイメージをローカル Docker で実行する。
...
.EXAMPLE
    .\Run-Local.ps1
    .\Run-Local.ps1 -Port 18080 -Password s3cret
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "イメージが見つかりません: $imageRef`n`nローカルでビルドするには次を実行してください:`n  mise run build-image"
}
